...
#### 代码
```go
import "github.com/simonalong/gole/extend/orm"

// gorm：获取默认配置库实例
orm.NewGormClient()

// gorm：获取默认配置库实例，自定义配置
orm.NewGormClientWitConfig(gormConfig *gorm.Config)

// xorm：获取默认配置库实例
orm.NewXormClient()

// xorm：获取默认配置库实例，自定义参数
NewXormClientWithParams(params map[string]string)
```
#### 全部配置
```yaml
base:
