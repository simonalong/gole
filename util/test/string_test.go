...
func TestBigCamelToMiddleLine2(t *testing.T) {
	originalStr := "DataBaseUSER"
	newStr := util.BigCamelToMiddleLine(originalStr)
	assert.Equal(t, newStr, "data-base-uSER")
}
...
func TestUpperLine(t *testing.T) {
	originalStr := "dataBaseUser"
	newStr := util.UnderLine(originalStr)
	assert.Equal(t, "data_base_user", newStr)
}
